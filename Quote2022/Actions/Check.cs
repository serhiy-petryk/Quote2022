    ...
    public static class Check
    {
        public static void MinuteYahoo_ErrorCheck(string[] zipFiles, Action<string> showStatusAction)
        {
            ...
            foreach (var zipFile in zipFiles)
            {
                ...
                var quotes = QuoteLoader.MinuteYahoo_GetQuotesFromZipFiles(showStatusAction, new [] {zipFile}, false, false);
                ...
                var priceErrorFileName = Settings.MinuteYahooLogFolder + $"errorsPrice_{folderTimeStamp}.txt";
                ...
                var volumeErrorFileName = Settings.MinuteYahooLogFolder + $"errorsVolume_{folderTimeStamp}.txt";
                ...
                var splitErrorFileName = Settings.MinuteYahooLogFolder + $"errorsSplit_{folderTimeStamp}.txt";
            ...
            showStatusAction( $"MinuteYahoo_ErrorCheck FINISHED! Found {(priceErrorCount - 2) / 2}, {splitErrorCount - 2}, {volumeErrorCount - 2} errors. Error logs in folder: {Settings.MinuteYahooLogFolder}");
        ...
        public static void MinuteYahoo_SaveLog(string[] zipFiles, Action<string> showStatusAction)
        {
            ...
            foreach (var zipFile in zipFiles)
            {
                ...
                var ss = Path.GetFileNameWithoutExtension(zipFile).Split('_');
                var folderTimeStamp = ss[ss.Length - 1];
                var logFileName = Settings.MinuteYahooLogFolder + $"log_{folderTimeStamp}.txt";
                if (File.Exists(logFileName))
                    File.Delete(logFileName);
            ...
            showStatusAction($"MinuteYahoo_SaveLog FINISHED! See log(s) in folder: {Settings.MinuteYahooLogFolder}");
        }
